...
current_users = defaultdict(tuple)  # key: username, value: room_id
...
def create_room(data):
    ...
    messaging({'message': user_id + ' just joined the room!', 'type': 'admin', 'name': user_id})
...
def join_room(data):
    ...
    if room_id in rooms:
        ...
        if user_id not in current_users:
            ...
            players = rooms[room_id]
            questions = room_questions[room_id]
            convo = user_id + ' just joined room ' + room_name + '!'

            messaging({'message': convo, 'type': 'admin', 'name': user_id})
...
def leave_room(data):
    ...
    if len(rooms[room_id]) == 0:
        ...
        room_name = room_name_pairs1[room_id]
        print("User " + user_id + " disconnected from " + str(room_id) + ". The users in the current room " +  str(room_id) + " are: ")
    ...
    messaging({'message': user_id + ' just left the room!', 'type': 'admin', 'name': user_id})
    ...
    socketio.server.leave_room(request.sid, room_id)
    print(rooms)
...
def messaging(data):
    ...
    if user_id in current_users:
        ...
        tmp = {'message': msg, 'user': user_id, 'name': user_id, 'type': msg_type, 'room_name': room_name}
        chat_logs[room_id].append(tmp)

        print('Received message from ' + user_id + ': ' + msg + ' in room ' + room_name)
...
def send_submission(data):
    ...
    if user_id in current_users:
        ...
        if not submission_status:
            msg = user_id + ' submitted question ' + str(id+1) + '!'

            # set status to 1: started but unsuccessful
            user_question_status[room_id][user_id][id] = 1
            messaging({'message': msg, 'type': 'submission', 'name': user_id})
        else:
            ...
            msg = user_id + ' completed the problem ' + str(id+1) + ' in ' + language + ', beat ' + str(percentile) + '% of users!'
            messaging({'message': msg, 'type': 'submission', 'name': user_id})
            ...
            if len(set(user_question_status[room_id][user_id])) == 1 and list(set(user_question_status[room_id][user_id]))[0] == 2:
                msg = user_id + ' finished the contest!'
                messaging({'message': msg, 'type': 'submission', 'name': user_id})
